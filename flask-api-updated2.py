...
# --- Prompt Builder ---
def build_prompt(user_message, history=None, mode="text", image_path=None):
    history_text = ""
    if history:
    ...
    if mode == "vision" and image_path:
        base_instruction += (
            f" You can also describe or analyze the uploaded image located at '{image_path}' "
            "if relevant, then continue your explanation in clear spoken sentences."
        )
...
def generate():
    try:
        ...
        prompt = build_prompt(user_message, convo['history'], mode, image_path)
